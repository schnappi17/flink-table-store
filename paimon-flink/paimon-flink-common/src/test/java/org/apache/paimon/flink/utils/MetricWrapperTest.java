...
public class MetricWrapperTest {
    ...
    public void testGaugeWrap() {
        ...
        org.apache.flink.metrics.Gauge<Long> flinkGauge = metricGroup.gauge("testGauge", new GaugeMetricMutableWrapper(longGauge));
        assertThat(flinkGauge.getValue()).isEqualTo(5L);

        // Lambda gauge
        Gauge<Long> lambdaGauge = () -> 6L;
        flinkGauge = metricGroup.gauge("testLambdaGauge", new GaugeMetricMutableWrapper(lambdaGauge));
    ...
    public void testCounterWrap() {
        ...
        org.apache.flink.metrics.Counter flinkCounter = metricGroup.counter("testCounter", new CounterMetricMutableWrapper(myCounter));
        myCounter.inc();
        ...
        assertThrows(
                UnsupportedOperationException.class, () -> flinkCounter.inc());
    }

    @Test
    public void testHistogramWrap() {
        Histogram myHistogram = new DescriptiveStatisticsHistogram(10000);
        org.apache.flink.metrics.Histogram flinkHistogram = metricGroup.histogram("testHistogram", new HistogramMetricMutableWrapper(myHistogram));
